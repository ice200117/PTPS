...
QList<Task *> JobResolver::getResolveResOnTaskList( QString strJobID )
{
	...
	for (int iNodeIndex = 0;iNodeIndex < childNodeList.size();iNodeIndex++)
	{
		...
		//int tmpUid = qrand() * QTime::currentTime().msec();

		long long tmpUid = QDateTime::currentDateTimeUtc().toMSecsSinceEpoch();
		//int iTmpUid = tmpUid % 2000;
		//tmpUid = tmpUid * 1000 + QDateTime::currentDateTimeUtc().toString("zzz").toInt();

		Task * newTask = new Task;
		newTask->status = WAITING;
		newTask->taskId = tmpUid;
        newTask->taskInfo = curTaskInfo;
		newTask->m_seconds = 1800000;
...
qlonglong JobResolver::getConditionOnceMillisecond(QString strJobID)
{
	...
	if (!childNodeList.isEmpty())
	{
		...
		else
		{
			...
			qllMillisecond = tmpStrLst[0].toInt() * 60 * 1000;
			qllMillisecond = qllMillisecond + tmpStrLst[1].toInt() * 1000;
			qllMillisecond = qllMillisecond + tmpStrLst[2].toInt();
			return qllMillisecond;
		}
	}
...
qlonglong JobResolver::getConditionCycleMillisecond(QString strJobID)
{
	Logger *p = &(Logger::getInstance());
	...
	QDomNodeList childNodeList;
	childNodeList = xmlDomDoc.elementsByTagName( "Cycle" );
	if (!childNodeList.isEmpty())
	{
		QDomNode tmpNode = childNodeList.at(0);
		QDomElement tmpElem = tmpNode.toElement();
	...
	p->writeInfo(QString("No Cycle type match on '%1'").arg(strJobID));
	return qllMillisecond;
}

...
#include "../Condition4CycleTimer/Condition4CycleTimer.h"
...
ConditionTrigger::ConditionTrigger(void)
{
	...
	m_listConditionObj.push_back(new Condition4CycleTimer);
...
QString ConditionTrigger::initConditionTrigger()
{
	...
	for (int i = 0;i < m_listConditionObj.size();i++)
	{
		...
		connect(m_listConditionObj[i],SIGNAL(signalExecJobOnCondition(QString)),this,SIGNAL(signalExecJob(QString)),Qt::DirectConnection);
	}

...
s_broker_worker_msg (broker_t *self, zframe_t *sender, zmsg_t *msg)
{
    ...
    worker_ready = (zhash_lookup (self->workers, id_string) != NULL);
    free (id_string);
...
s_broker_client_msg (broker_t *self, zframe_t *sender, zmsg_t *msg)
{
    ...
    &&  memcmp (zframe_data (service_frame), "mmi.", 4) == 0) {
        ...
        if (zframe_streq (service_frame, "mmi.service")) {
            ...
            service2 = (service_t *) zhash_lookup (self->services, name);
            return_code = service2 && service2->workers? "200": "404";
            free (name);
        }
        else
            return_code = "501";
...
s_service_require (broker_t *self, zframe_t *service_frame)
{
    ...
    service = (service_t *) zhash_lookup (self->services, name);
    if (service == NULL) {
...
s_service_destroy (void *argument)
{
    ...
    while (zlist_size (service->requests)) {
        msg = (zmsg_t *)zlist_pop (service->requests);
        zmsg_destroy (&msg);
    }
...
s_service_dispatch (service_t *self, zmsg_t *msg)
{
    ...
    while (zlist_size (self->waiting) && zlist_size (self->requests)) {
        worker = (worker_t *)zlist_pop (self->waiting);
        zlist_remove (self->broker->waiting, worker);
        msg2 = (zmsg_t *)zlist_pop (self->requests);
        s_worker_send (worker, MDPW_REQUEST, NULL, msg2);
        zmsg_destroy (&msg2);
    }
}
...
s_worker_require (broker_t *self, zframe_t *identity)
{
    ...
    worker = (worker_t *) zhash_lookup (self->workers, id_string);

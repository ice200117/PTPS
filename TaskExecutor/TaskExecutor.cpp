...
TaskExecutor::~TaskExecutor(void)
{
	//QMutexLocker locker(&m_Mut4lstRunProPID);
	//foreach (qint64 * pIn,m_lst4RunProPID)
	//{
	//	delete pIn;
	//}
}
...
int TaskExecutor::loadLib()
{
    ...
    if(pid < 0){
        ...
        printf("taskId %lld is running on pid %d !\n", m_task.taskId,  pid);
...
int TaskExecutor::startProcess()
{
	...
	m_iTimeOutNum = m_task.m_seconds;
	QFileInfo qfiExec(program);
	QString qstrAbsPath = qfiExec.absolutePath();
	QProcessEnvironment env = QProcessEnvironment::systemEnvironment();
	env.insert("PATH", env.value("Path") + ";" + qstrAbsPath);
	qint64 *qiPID = new qint64();

	if (QProcess::startDetached ( program, arguments, qstrAbsPath, qiPID))
	{
		m_pRunProPID = qiPID;

		printf("%s startDetached successful!\n", program.toStdString().c_str());
		log(" startDetached successful!");

		return waitForProcessDone();
	}
	else
	{
		printf("%s startDetached failed!\n", program.toStdString().c_str());
		log(" startDetached failed!");
...
void TaskExecutor::log(QString stat)
{
    ...
    logStr += " taskId ";
	logStr += QString("%1  ").arg(m_task.taskId);
    logStr += stat;
    l->writeInfo(logStr);
}

int TaskExecutor::waitForProcessDone()
{
	if (m_qTimer) delete m_qTimer;
	m_qTimer = new QTimer;
	m_qTimer->setSingleShot(true);
	connect(m_qTimer,SIGNAL(timeout()),this,SLOT(slotHandleTimer()),Qt::QueuedConnection);
	m_qTimer->start(m_iTimeOutNum);
...
void TaskExecutor::slotHandleTimer()
{
	log(" timeout!");
	killProcess();
} 

bool TaskExecutor::ProcessExist()
{
#ifdef _WINDOWS
	...
	{
		QProcess myTaskList;
		myTaskList.setEnvironment(QProcess::systemEnvironment());
		myTaskList.start("tasklist");
		myTaskList.waitForStarted();
		myTaskList.waitForFinished();
		readAllStandardOutput = myTaskList.readAllStandardOutput();
		myTaskList.close();
	}
	...
	for (int i = 4;i < strListTemp.size();i++)
	{
		...
		if (singleProcInfo.size()>2 && 0 != m_pRunProPID && (singleProcInfo[1].toLongLong() == *m_pRunProPID))
		{
			return true;
		}
	}
    ...
    QByteArray readAllStandardOutput;
    {
        QProcess myTaskList;
        myTaskList.setEnvironment(QProcess::systemEnvironment());
        //myTaskList.start("ls -1 /proc");
        myTaskList.start("ps -eo pid");
        myTaskList.waitForStarted();
        myTaskList.waitForFinished();
        readAllStandardOutput = myTaskList.readAllStandardOutput();
        myTaskList.close();
    ...
    QString strTemp = QString(readAllStandardOutput);
    QStringList strListTemp = strTemp.split(QRegExp("\\s+"));
    for (int i = 0;i < strListTemp.size();i++)
    {
        if(0 != m_pRunProPID && strListTemp[i].toLongLong() == *m_pRunProPID){
            return true;
        }
    }
    return false;
...
void TaskExecutor::killProcess()
{
	...
	QString strCommand = QString("kill -9 %1").arg(*m_pRunProPID);
    myKillProcess.start(strCommand);
